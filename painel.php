...
<div id="box">
   ...
   <div id="header">
    ...
    <div id="content">
		...
		IF($tipo=="aluno")
		{
			$query = "SELECT COUNT(n_req) FROM req_almocos WHERE n_semana='".$sem."' AND  n_proc='".$user."'";
			$query_result = mysql_query ($query);
			while($info = mysql_fetch_assoc($query_result))
			{
				$teste = $info['COUNT(n_req)'];
			}
			$query4 = "SELECT COUNT(n_req) FROM req_almocos WHERE n_semana='".$sem2."' AND  n_proc='".$user."'";
			$query_result4 = mysql_query ($query4);
			while($info2 = mysql_fetch_assoc($query_result4))
			{
				$teste2 = $info2['COUNT(n_req)'];
			}
			$query2 = "SELECT COUNT(n_req) FROM req_almocos WHERE almocou='0' AND  n_proc='".$user."' AND n_semana < '".$sem."'";
			$query_result2 = mysql_query ($query2);
			while($info3 = mysql_fetch_assoc($query_result2))
			{
				$teste3 = $info3['COUNT(n_req)'];
			}
			$max = 4 - $teste3;
			IF ($max <=0)
			{
		...
		IF($tipo=="func")
		{
			$query = "SELECT COUNT(n_req) FROM req_almocos WHERE n_semana='".$sem."' AND  id_func='".$user."'";
			$query_result = mysql_query ($query);
			while($info = mysql_fetch_assoc($query_result))
			{
				$teste = $info['COUNT(n_req)'];
			}
			$query4 = "SELECT COUNT(n_req) FROM req_almocos WHERE n_semana='".$sem2."' AND  id_func='".$user."'";
			$query_result4 = mysql_query ($query4);
		...
		IF($tipo=="prof")
		{
			$query = "SELECT COUNT(n_req) FROM req_almocos WHERE n_semana='".$sem."' AND  id_prof='".$user."'";
			$query_result = mysql_query ($query);
			while($info = mysql_fetch_assoc($query_result))
			{
				$teste = $info['COUNT(n_req)'];
			}
			$query4 = "SELECT COUNT(n_req) FROM req_almocos WHERE n_semana='".$sem2."' AND  id_prof='".$user."'";
			$query_result4 = mysql_query ($query4);
			while($info = mysql_fetch_assoc($query_result4))
			{
				$teste2 = $info['COUNT(n_req)'];
        ...
        <?php $_SESSION['nome'] = $nome; ?>
        <h2>Configure aqui a sua conta</h2>
        </br>
        <p> Tem <m><?php echo $teste; ?></m> refeiçoes marcadas para esta semana. </p>
        <p> Tem <m><?php echo $teste2; ?></m> refeiçoes marcadas para a proxima semana. </p>

        <p> Ja faltou a <m><?php echo $teste3;?></m> refeições até à semana passada</p>
        <p> Pode dar mais <m><?php echo $max; ?></m> falta(s) até ser impedido de almoçar na escola </p>
         <p> Consulte <a href="/regl.php"> aqui</a> o regulamento das refeições no INED Maia. </p>
         <br>
         <br>
         <br>
         <br>
         ...
         <br>
                  ...
                  <p> Mudar Password <a href="/mudapass.php"> aqui </a> </p>
   		</div>
    	<div id= "sidebar">
        <h1>Esta Semana:</h1>
        <br />
