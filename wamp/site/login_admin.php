...
if (isset($_POST['n_proc'])) {
  ...
  $LoginRS__query=sprintf("SELECT id_admin, pw_admin FROM admin WHERE id_admin='".$loginUsername."' AND pw_admin='".$password."'",
...
if (isset($_POST['n_proc'])) {
  ...
  $LoginRS__query=sprintf("SELECT id_admin, pw_admin FROM `admin` WHERE id_admin=%s AND pw_admin=%s",
    GetSQLValueString($loginUsername, "text"), GetSQLValueString($password, "text")); 
