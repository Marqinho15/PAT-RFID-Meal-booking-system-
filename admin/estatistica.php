...
<div id="box">
   ...
   <div id="header">
    ...
    <div id="content">
		...
		$query = "SELECT COUNT(n_req) FROM req_almocos";
			
		$query_result = mysql_query ($query);
		while($info = mysql_fetch_assoc($query_result))
		{
			$teste = $info['COUNT(n_req)'];
		}
		$query2 = "SELECT COUNT(n_req) FROM req_almocos WHERE almocou=0";
			
		$query_result2 = mysql_query ($query2);
		while($info = mysql_fetch_assoc($query_result2))
		{
			$teste2 = $info['COUNT(n_req)'];
		}
		$query3 = "SELECT COUNT(n_req) FROM req_almocos WHERE almocou=0 AND mes='".$mes."'";
		$query_result3 = mysql_query ($query3);
		while($info = mysql_fetch_assoc($query_result3))
		{
			$teste3 = $info['COUNT(n_req)'];
		}
		$query4 = "SELECT COUNT(n_req) FROM req_almocos WHERE mes='".$mes."'";
		$query_result4 = mysql_query ($query4);
		while($info = mysql_fetch_assoc($query_result4))
		{
			$teste4 = $info['COUNT(n_req)'];
		}
		$query5 = "SELECT COUNT(n_req) FROM req_almocos WHERE almocou=0 AND mes='".$mesa."'";
		$query_result5 = mysql_query ($query5);
		while($info = mysql_fetch_assoc($query_result5))
		{
			$teste5 = $info['COUNT(n_req)'];
		}
		$query6 = "SELECT COUNT(n_req) FROM req_almocos WHERE mes='".$mesa."'";
		$query_result6 = mysql_query ($query6);
		while($info = mysql_fetch_assoc($query_result6))
		{
			$teste6 = $info['COUNT(n_req)'];
		}
        ...
        <p> Aqui pode verificar as principais estatísticas em relação à utilização das senhas adiquiridas</p>
        <p> Total de requisições de senhas do ano:  <m><?php echo $teste;?>  </p> 
        <p> Total de senhas falhadas do ano: <m><?php echo $teste2; ?></p>
        <p> Total de requisições de senhas deste mes: <m><?php echo $teste4; ?></p>
        <p> Total de senhas falhadas deste mes: <m><?php echo $teste3; ?> </p>
        <p> Total de requisições de senhas do mes passado: <m><?php echo $teste6; ?> </p>
        <p> Total de senhas falhadas do mes passado: <m><?php echo $teste5; ?></p>
        <?php
        $query8 = "SELECT `n_proc`, `id_prof`, `id_func`, `dia` FROM `req_almocos` WHERE almocou='0' AND n_semana='".$sem3."'"; 
		...
		{
			echo "<p> ola". $row['n_proc']. " - ". $row['id_prof']. " - ". $row['id_func']. " - ". $row['dia'];
			echo "OLA<br />";
		}
}
else
